...
要求 Node.js 20.19+ 或 22.12+（也支持更新版本）和 npm；该范围与锁定的 Vite 版本一致。
...
端到端套件在 1440×1000 桌面、1024×768 平板和 390×844 移动视口运行，保留失败 trace，并把浏览器 `pageerror` 与 `console.error` 视为失败。音频测试断言引擎调用和界面播放状态，不比较声波。
